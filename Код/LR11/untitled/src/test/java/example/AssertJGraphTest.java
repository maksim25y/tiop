...
public class AssertJGraphTest {
    ...
    String txtEdgesConnectedSecond = """
            1 2 false
            """;
    ...
    int[][] arrayFromFileConnectedSecond = {
            {1},
            {1}
    };
    ...
    void graphReaderTxt() throws IOException {
        try(StringReader reader = new StringReader(txtEdgesConnected)) {
    ...
    void isConnected() throws IOException {
        try(StringReader reader = new StringReader(htmlLineConnected); StringReader secondReader = new StringReader(txtEdgesConnectedSecond)) {
            Graph graph = new Graph(new GraphReaderWriterHtml(), new GraphReaderWriterTxt());
            graph.readFromFile(reader);
            Assertions.assertThat(graph.isConnected())
                    .as("Граф не связный, ожидалось обратное!")
                    .isTrue();
            Graph secondGraph = new Graph(new GraphReaderWriterTxt(), new GraphReaderWriterTxt());
            secondGraph.readFromFile(secondReader);
            Assertions.assertThat(secondGraph.isConnected()).isTrue();
        }
    }
    ...
    @Test
    void isDirected() {
        Edge edgeDirected = new Edge(1, 2, true);
        Edge edgeNotDirected = new Edge(1, 2, false);
        Assertions.assertThat(edgeDirected.isDirected()).isTrue();
        Assertions.assertThat(edgeNotDirected.isDirected()).isFalse();
    }
    ...
    void equalsEdge() {
        Edge edgeDirected = new Edge(1, 2, true);
        Edge edgeNotDirected = new Edge(1, 2, false);
        Edge firstCopy = edgeDirected;
        Object notEdge = null;

        Assertions.assertThat(edgeDirected).isNotEqualTo(edgeNotDirected);
        Assertions.assertThat(edgeDirected).isEqualTo(firstCopy);
        Assertions.assertThat(edgeDirected).isNotEqualTo(notEdge);
    }
